...
function smoothCaretMotion(backspace = "", typing = document.getElementById('typing')) {
    ...
    if (0.1+Math.ceil(xvalue) >= Math.trunc(typing.offsetWidth/14)) {
        xvalue = xvalue-(noOfFirstLineCharacter*1.8*Math.trunc(1.8 * typing.selectionStart / Math.trunc(typing.offsetWidth / 14)))
        console.log("next line occur")
        console.log("next line occur")
        console.log("next line occur")
        console.log("next line occur")
        console.log("next line occur")
        console.log(0.1+Math.ceil(xvalue) +" || "+ Math.trunc(typing.offsetWidth/14))
    }
    if (0 < Math.trunc(3 * Math.trunc(1.8 * typing.selectionStart / Math.trunc(typing.offsetWidth / 14)))) {
        ...
        if (0 == Math.trunc(3 * Math.trunc(typing.selectionStart / Math.trunc(typing.offsetWidth / 14)))) {
...
const Typing = () => {
    ...
    // function startCountdown(setTimeout=0) {
    //     setInterval((setTimeout) => {
    //         setTimeout=(setTimeout>0)?setTimeout-1000:clearInterval();
    ...
    return (
        <>
            <HelmetProvider>
                ...
                <Box className="text-white flex gap-x-[20vw] my-5 text-4xl justify-center">
                ...
                <Box id="typingContainer" className="border-transparent focus:outline-none border-2 h-[50vh] m-5 mx-10 rounded-lg" onClick={() => typingContainer.classList.remove("blur-md")} onBlur={() => typingContainer.classList.add("blur-md")}>
                    <div id='caret' className="w-[5px] h-[3.5rem] flex flex-col rounded-md absolute z-10 bg-blue-500"></div>
                    <textarea className="rounded-lg bg-blue-300 focus:outline-none text-5xl w-[100%] h-[100%] resize-none caret-transparent text-blue-600 text-opacity-100 bg-opacity-100 selection:bg-transparent" style={{ wordBreak: 'break-all' }} onScroll={scrolled} name="typing" id="typing" spellCheck="false" ref={elementRef} onChange={handleTyping} autoFocus={true} value={Letter}></textarea>
                    <textarea className="rounded-lg bg-orange-300 focus:outline-none text-5xl w-[100%] h-[100%] resize-none caret-transparent select-none placeholder:text-white relative top-[-50.4vh] overflow-hidden bg-opacity-0 opacity-40" style={{ wordBreak: 'break-all' }} onClick={focusTyping} name="placeholder" id="placeholder" spellCheck="false" placeholder={placeholderText} ></textarea>
                </Box>
